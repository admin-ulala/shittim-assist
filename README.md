...
日常模块规划与编写入口见 [自动化模块文档](docs/AUTOMATION.md)：咖啡厅、日程、社交、制造、商店已有独立语义流程草案和离线执行器；页面适配器尚未接入，不能直接执行日常。其余模块保留明确占位。

- Flutter 自适应界面：总览、设备、任务计划、日志、设置。
- PC：USB/模拟器 ADB 列表、网络连接、无线配对、截图、点击、滑动与返回 API。
- Android：屏幕采集、通知停止、无障碍输入 API、图标悬浮工作台和双向配置同步。
...
在设备页开启悬浮控制，切到 B 服后点击应用图标。展开面板提供运行、任务、配置三个页签，可运行只读诊断、调整常用配置并同步主界面。后台保留执行引擎；日常流程尚未适配，真实手势仍待验证。

Windows ZIP 与 Android debug APK 可从[自动构建记录](https://github.com/admin-ulala/shittim-assist/actions)的 Artifacts 区域下载（GitHub 可能要求登录）。不同 CI 构建的 debug 签名可能不同；测试版本之间可能需要备份配置后卸载旧助手再安装，并重新授权。正式稳定签名尚未配置。
...
在「设备连接」依次开启无障碍、授权屏幕采集、开启悬浮控制。切换到国服 B 服，点击应用图标悬浮球展开面板，再点「开始只读诊断」。悬浮球可拖动；面板支持暂停、继续、停止和关闭。关闭悬浮球会取消任务，屏幕采集可在系统通知中单独停止。
...
主界面与悬浮面板按可用窗口自适应，不要求 1280×720 / DPI 240。较矮窗口使用底部导航，面板内部可滚动。开发接口支持按实际截图尺寸缩放参考坐标、模板和识别区域；DPI 只影响 UI，不参与游戏像素坐标换算。

宽高比不同的设备不能只做横纵拉伸：需验证游戏内容区域或提供对应布局资源。横竖屏、黑边、超宽屏的布局测试通过不等于全部游戏识别已适配，具体结果见实现状态文档。
